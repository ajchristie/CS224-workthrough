...
from trainer.util import Progbar, minibatches
...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self):
        ...
        _, final_state = tf.nn.dynamic_rnn(cell, x, dtype=tf.float32) #since no intial state
...
if __name__ == "__main__":
    ...
    #parser = subparsers.add_parser('predict', help='Plot prediction behavior of different cells')
    parser.add_argument('-c', '--cell', choices=['rnn', 'gru', 'lstm'], default='rnn', help="Type of cell to use")
    parser.add_argument('-g', '--clip_gradients', action='store_true', default=False, help="If true, clip gradients")
    parser.add_argument('-l', '--max-length', type=int, default=20, help="Length of sequences to generate")
    parser.add_argument('-o', '--output-prefix', type=str, default="q3", help="Length of sequences to generate")
    parser.add_argument('-j', '--job-dir', type=str, help="Where to put the output (this is a gcloud thing)")
    parser.set_defaults(func=do_sequence_prediction)
    ...
    if ARGS.func is None:
        ...
        #learn_runner.run(experiment_fn=ARGS.func(ARGS), output_dir=job_dir)
        ARGS.func(ARGS)
