...
import numpy as np
from trainer.util import read_conll, one_hot, window_iterator, ConfusionMatrix, load_word_vector_mapping
from trainer.defs import LBLS, NONE, LMAP, NUM, UNK, EMBED_SIZE
...
class ModelHelper(object):
    ...
    def save(self, path):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        with file_io.FileIO(os.path.join(path, "features.pkl"), "w") as f:
            pickle.dump([self.tok2id, self.max_length], f)
        # if path.startswith('gs://'):
        #     npath = path[5:]
        #     with file_io.FileIO(os.path.join(npath, "features.pkl", "w")) as f:
        #         pickle.dump([self.tok2id, self.max_length], f)
        # # Save the tok2id map.
        # else:
        #     with file_io.FileIO(os.path.join(path, "features.pkl"), "w") as f:
    ...
    def load(cls, path):
        ...
        # Save the tok2id map.
        with file_io.FileIO(os.path.join(path, "features.pkl"), "r") as f:
            tok2id, max_length = pickle.load(f)
        # if path.startswith('gs://'):
        #     npath = path[5:]
        #     with file_io.FileIO(os.path.join(npath, "features.pkl")) as f:
        #         tok2id, max_length = pickle.load(f)
        # else :
        #     with file_io.FileIO(os.path.join(path, "features.pkl")) as f:
        #         tok2id, max_length = pickle.load(f)
        return cls(tok2id, max_length)

def load_and_preprocess_data(args):
    logger.info("Loading training data...")
    with file_io.FileIO(args.data_train, "r") as f:
        train = read_conll(f)
    logger.info("Done. Read %d sentences", len(train))
    logger.info("Loading dev data...")
    with file_io.FileIO(args.data_dev, "r") as f:
        dev = read_conll(f)
...
def load_embeddings(args, helper):
    ...
    embeddings[0] = 0.
    vocab_file = file_io.FileIO(args.vocab, "r")
    vector_file = file_io.FileIO(args.vectors, "r")
    for word, vec in load_word_vector_mapping(vocab_file, vector_file).items():
        word = normalize(word)
