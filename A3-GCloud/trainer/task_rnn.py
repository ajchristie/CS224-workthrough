...
from trainer.util import print_sentence, write_conll, read_conll
from trainer.data_util import load_and_preprocess_data, load_embeddings, ModelHelper
from trainer.ner_model import NERModel
from trainer.defs import LBLS
from trainer.q2_rnn_cell import RNNCell
from trainer.q3_gru_cell import GRUCell
...
class Config:
    ...
    def __init__(self, args):
        ...
        if "job_dir" in args:
            ...
            self.output_path = args.job_dir
        else:
            self.output_path = "results/{}/{:%Y%m%d_%H%M%S}/".format(self.cell, datetime.now())
        self.model_output = self.output_path + "_model.weights"
        self.eval_output = self.output_path + "_results.txt"
        self.conll_output = self.output_path + "_{}_predictions.conll".format(self.cell)
        self.log_output = self.output_path + "_log"
...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) <= max_length:
            sent = sentence + [zero_vector] * (max_length - len(sentence))
            lbls = labels + [zero_label] * (max_length - len(labels))
            mask = [True] * len(sentence) + [False] * (max_length - len(labels))
            ret.append((sent, lbls, mask))
        else:
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable(name="b2", shape=(self.config.n_classes), initializer=tf.constant_initializer(0.))
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        loss_unred = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds)
        loss = tf.reduce_mean(tf.boolean_mask(loss_unred, self.mask_placeholder))
        ### END YOUR CODE
        return loss
    ...
    def preprocess_sequence_data(self, examples):
        def featurize_windows(data, start, end, window_size = 1):
            ...
            ret = []
            for sentence, labels in data:
                from trainer.util import window_iterator
                sentence_ = []
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            ...
            if report:
                ...
                with file_io.FileIO(model.config.conll_output, 'w') as f:
                    write_conll(f, output)
                with file_io.FileIO(model.config.eval_output, 'w') as f:
...
def do_evaluate(args):
    ...
    with file_io.FileIO(args.data, "r") as f:
        input_data = read_conll(f)
    embeddings = load_embeddings(args, helper)
    config.embed_size = embeddings.shape[1]
...
if __name__ == "__main__":
    ...
    #parser = subparsers.add_parser('train', help='')
    parser.add_argument('-dt', '--data-train', type=str, default="data/train.conll", help="Training data")
    parser.add_argument('-dd', '--data-dev', type=str, default="data/dev.conll", help="Dev data")
    parser.add_argument('-v', '--vocab', type=str, default="data/vocab.txt", help="Path to vocabulary file")
    parser.add_argument('-vv', '--vectors', type=str, default="data/wordVectors.txt", help="Path to word vectors file")
    parser.add_argument('-c', '--cell', choices=["rnn", "gru"], default="rnn", help="Type of RNN cell to use.")
    parser.add_argument('-j', '--job-dir', type=str, help="Where to put the output (this is a gcloud thing)")
    parser.set_defaults(func=do_train)
