...
## I added this; I ain't takin this class; I ain't need to follow the rules
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in range(sen_length):
            if i - window_size >= 0 and i + window_size < sen_length:
                window = sentence[i - window_size: i + window_size + 1]
            elif i - window_size < 0 and i + window_size < sen_length:
                window = [start] * (window_size - i) + sentence[:i + window_size + 1]
            elif i - window_size >= 0 and i + window_size >= sen_length:
                window = sentence[i - window_size:] + [end] * (i + window_size + 1 - sen_length)
            else:
                window = [start] * (window_size - i) + sentence + [end] * (i + window_size + 1 - sen_length)
            windowed_data.append((list(chain.from_iterable(window)), labels[i]))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embeds = tf.Variable(self.pretrained_embeddings)
        features = tf.nn.embedding_lookup(embeds, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(shape=(self.config.n_window_features * self.config.embed_size, self.config.hidden_size), initializer=tf.contrib.layers.xavier_initializer(), name='W')
        U = tf.get_variable(shape=(self.config.hidden_size, self.config.n_classes), initializer=tf.contrib.layers.xavier_initializer(), name='U')
